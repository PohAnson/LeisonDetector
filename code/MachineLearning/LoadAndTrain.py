...
weights_used = False
weight_path = r"output/models/bests/model.02-1.60.h5"
...
def getSubsetNames(df: pd.DataFrame, lesion_type: int, num: int):
    """Get the filenames of the lesion.

    Args:
        df (pd.DataFrame): Data frame containing field 'File_name' and 'Coarse_lesion_type'
...
if __name__ == "__main__":
    ...
    fns_list = []
    for i in range(1, 9):
        fns_list.extend(getSubsetNames(df, i, 5))
    ...
    if config.SAVE_ON_EPOCH:
        cbs.append(
            tf.keras.callbacks.ModelCheckpoint(
                filepath="./output/models/bests/model.{epoch:02d}-{loss:.2f}.h5",
    ...
    print(model.summary())
    H = model.fit(x=datagen, epochs=config.NUM_EPOCHS, workers=16, max_queue_size=4)
